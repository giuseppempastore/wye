...
class StorageSettings:
    ...
    def from_env(cls):
        ...
        return cls(provider,os.getenv("WYE_STORAGE_ENDPOINT") or None,required["WYE_STORAGE_BUCKET"],os.getenv("WYE_STORAGE_REGION") or ("auto" if provider=="r2" else "us-east-1"),required["WYE_STORAGE_ACCESS_KEY"],required["WYE_STORAGE_SECRET_KEY"],os.getenv("WYE_STORAGE_FORCE_PATH_STYLE","false").lower() in {"1","true","yes"},_positive("WYE_STORAGE_UPLOAD_TTL_SECONDS",900),_positive("WYE_STORAGE_READ_TTL_SECONDS",300),_positive("WYE_STORAGE_MAX_IMAGE_BYTES",15728640),_positive("WYE_STORAGE_CLEANUP_AFTER_SECONDS",1800),os.getenv("WYE_STORAGE_PUBLIC_ENDPOINT") or None)
