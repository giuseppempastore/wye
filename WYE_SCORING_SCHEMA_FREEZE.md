...
execution persistence: Phase 7.6.4B-2 COMPLETED + COMMITTED
```
...
execution semantic digest + selection/result/trace roots
...
| `publication_bundle_digest` | Execution semantic root and selection/result/trace roots | projection rows, publish time, request key | Atomic scientific publication identity |
...
| `entity_type` | Through 0020: `protocol, protocol_version, artifact, artifact_location, evidence_snapshot`; 0021 adds `evaluation_execution, evaluation_result` |
| entity FK | exactly one concrete matching FK; 0021 adds execution/result, never a generic entity id |
| `event_type` | Existing closed vocabulary plus `counterfactual_authorized` in 0021 |
...
artifact-location events; revision 0020 adds evidence snapshot. Revision 0021
adds execution/result primary and related FKs and expands the CHECKs after those
parents exist. Attempts are operational and publication is governed through its
execution/result. No temporarily unenforced polymorphic reference is allowed.
...
target_type
target_logical_key
substance_id or ingredient_id (exactly one concrete traversal FK)
...
The column sketch in this historical 7.6.1 section is superseded for 0021 by
`WYE_EXECUTION_PERSISTENCE_FREEZE.md`. In particular, concrete engine-build
metadata belongs to attempts, semantic engine compatibility belongs to the
configuration artifact, concrete substance/ingredient FKs replace the
unconstrained current-row hint, and the semantic identity has its own canonical
artifact. The logical roots and mode semantics below remain applicable.
...
| `evidence_snapshot_id` | NOT NULL FK sealed snapshot |
| `target_type` | CHECK initially `substance, ingredient`; product deferred to a later schema checkpoint |
| `substance_id`, `ingredient_id` | exactly one concrete nullable FK matching `target_type` |
...
| `semantic_identity_artifact_id` | NOT NULL UNIQUE FK `scientific_evaluation_execution_identity/1` artifact |
...
applies to all modes; repeated verification uses new attempts and the one
immutable replay verification on the same semantic execution.
...
The later 7.6.4B-1 freeze narrows this preliminary physical sketch: projection
sets are 0022 rebuildable query state and are not a prerequisite or digest input
for 0021 scientific publication. The exact 0021 trace/publication columns and
bundle roots are defined in `WYE_EXECUTION_PERSISTENCE_FREEZE.md`.
...
| `result_digest`, `selection_digest`, `protocol_digest`, `snapshot_digest`, `input_digest` | NOT NULL 32-byte linkage proofs |
| `created_at` | audit timestamp |
...
| selection/result/trace/bundle digests | NOT NULL 32-byte fields |
...
versions. A `NORMAL`, `REFRESH` or `COUNTERFACTUAL` execution has one
publication. A `REPLAY` execution has none and completes through the bounded
verification frozen by 7.6.4B-1B.

### Rich replay reporting and request idempotency

The bounded 0021 `scientific_evaluation_replay_verifications` row supersedes
this preliminary sketch for the completion fact. The richer report below
remains a later 0022 reconciliation/support-level projection and is not required
to complete REPLAY.
...
For 0021, references below to domain projections or a projection manifest are
historical planning language. The authoritative atomic scientific bundle is
semantic execution + selection + result + trace. Projection generation and
activation occur independently in 0022 and cannot change publication identity.
...
2. Build all decision, result, component, trace and bundle
...
10. Insert result and components.
11. Insert trace.
...
15. Return publication identity after a fresh read. Asynchronous reconciliation
    handles only unreferenced objects, never missing canonical content. Query
    projections are built separately in 0022.

A canonical result exists only after step 14; user/API visibility is a separate
governance and rollout decision. Snapshot sealing
...
## Phase 7.6.4B-1 execution/publication schema freeze

`WYE_EXECUTION_PERSISTENCE_FREEZE.md` supersedes the preliminary 7.6.1 physical
sketch for migration 0021 and freezes:
...
One content-addressed execution-identity artifact produces the globally UNIQUE
`semantic_execution_digest`. The execution row binds explicit RESTRICT FKs to
protocol version, sealed snapshot, target artifact, mapping artifact when
applicable, input artifact and semantic configuration. Concrete engine build,
lease and failure provenance belong to append-only attempts.

0021 creates exactly execution, attempt, selection-decision, result,
result-component, trace, atomic-publication, replay-verification and
request-idempotency tables, and extends governance with concrete
execution/result relations. It does not create domain projections or rich
replay reports. The scientific publication bundle for `NORMAL`, `REFRESH` and
`COUNTERFACTUAL` is:
...
Result scientific status remains distinct from execution/attempt technical
status. Detailed scientific component payloads remain protocol/review-gated;
the persistence envelope introduces no numerical field or semantics.

The Phase 7.6.4B-1B amendment supersedes the earlier rule that REPLAY owns a new
publication. The 0019 registry remains globally content-addressed. A REPLAY
execution compares recomputed canonical selection/result/trace roots with the
exact historical comparison publication and inserts one immutable
`scientific_evaluation_replay_verifications` row plus
`scientific_evaluation_replay_verification/1`. `matched` and `mismatch` are
technical verification states; neither creates a new scientific result, trace,
selection-decision set or publication. Existing exclusive artifact ownership
for canonical result, trace and publication selection remains unchanged.
...
The documentation checkpoint itself implemented no migration. Phase 7.6.4B-2
has an uncommitted migration/test delta for
`0021_scientific_evaluation_publication`; it implements the nine-table REPLAY
amendment and is `COMPLETED + COMMITTED`. No execution/scoring or replay runtime
is implemented; local `wye` remains at 0017.
